...
`V.1.4.48` — fix: piloto Claude Account usava `allowed_tools=[]` (so remove pre-aprovacao) em vez de `tools=[]` (desabilita de verdade) para tentar rodar sem ferramentas — esgotava max_turns tentando usa-las e falhava com `error_max_turns` em toda analise real. V.1.4.47 (token corrompido por espaco), V.1.4.46 (ANTHROPIC_API_KEY herdada), V.1.4.45 (piloto em Analisar Transcricao) e V.1.4.44 (piloto Claude Account) confirmadas deployadas | extensao Chrome `V.1.4.3` | VPS antiga (`204.168.180.25`) deprecada
